// One possible farm implementation in MPI
// using double-buffering, i.e., overlapping
...
void Worker(int rank, int size) {
    ...
    while(true) {
		...
		// we could parallelize this loop using OpenMP (you must compile with -DTHREADED_WORKERS)
		for(int i=0;i<tasks_in_batch;i++){
			
			// simulate work
			int workload;
			std::memcpy(&workload, recv_buf[idx].data() + i*task_size, sizeof(workload));
...
void usage(char *argv[]) {
	std::printf("usage:\n mpirun -n <P> %s total_tasks bytes_x_task batch_size [prob=%f]\n srun --mpi=pmix -N <num-nodes> -n <P> %s total_tasks bytes_x_task batch_size [prob=%f]\n",
				argv[0], psuccess, argv[0], psuccess);
}
...
#else
	MPI_Init(&argc, &argv);  // equivalent to MPI_Init_thread(..., MPI_THREAD_SINGLE, ...);
#endif
