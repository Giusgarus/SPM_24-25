 ...
 *        - Posts two non-blocking 'MPI_Irecv' calls (double-buffered) to receive
 *          incoming task batches from the Emitter.  
 ...
 *            - Each Worker node applies 'active_delay()' to simulate CPU-intensive
 *              work and copies results into an output buffer.  
 *        - Sends results to the Collector (rank 'size-1') via non-blocking
 *          'MPI_Isend', using double-buffering to overlap sending and next receive.
 *
 *     3. Collector (rank size-1):
 *        - Posts two non-blocking 'MPI_Irecv' calls to collect result batches
 *          from any Worker.  
 *        - Uses 'MPI_Waitany' to handle incoming batches, accumulating 'total_tasks'
 *          and tracking counts per Worker.  
 *        - After receiving all tasks, prints per-Worker counts and elapsed time.
 *
 *   - The program requires at least 3 MPI processes and initializes MPI with
 *     'MPI_THREAD_MULTIPLE' to support concurrent MPI calls from FastFlow threads
 *      (MPI_THREAD_FUNNELED would be enough indeed).
 *
 * Usage:
 *   mpirun -n P ./Farm+ff_farm total_tasks task_size batch_tasks threads_per_worker [psuccess]
 *
 */
...
void Emitter(int size) {
	...
	auto prepare_and_send_batch= [&](Buf &buf, int rank) {
		...
		int error = MPI_Isend(buf.data, count*task_size,
							  MPI_BYTE, rank,
...
void Worker(int rank, int size) {
	...
	while(true) {
		...
		// TODO (optimization): use a ff_farm in which the Emitter receives the batch
		// from the MPI Emitter, the Workers compute one or more items of the batch
		// and send results to the local Collector, which sends the output batch
		// to the MPI Collector.
...
void Collector(int num_workers) {
    ...
    error = MPI_Irecv(buffers[0], buff_size, MPI_BYTE,
					  MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, requests);
    CHECK_ERROR(error);
    error = MPI_Irecv(buffers[1], buff_size, MPI_BYTE,
					  MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, requests+1);
    CHECK_ERROR(error);
    ...
    while (true) {
        ...
        error = MPI_Irecv(buffers[idx], buff_size, MPI_BYTE,
						  MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, requests+idx);
        CHECK_ERROR(error);
...
void usage(char *argv[]) {
	std::printf("usage: %s total_tasks bytes_x_task batch_size thread_x_worker [prob=%f]\n",
				argv[0], psuccess);
}
