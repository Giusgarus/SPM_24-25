// It shows how to mix MPI and OpenMP (hybrid programming)
// MPI should be properly initialized with MPI_Init_thread
//
#include <cstdio>
#include <string>
#include <mpi.h>
...
#pragma omp parallel for reduction(+:area) num_threads(nthreads)
    for (int i= 0; i < end; ++i) {
...
int main( int argc, char* argv[] ) {
	...
	if (argc==2  && std::string(argv[1])=="-h") {
		std::printf("use: %s [n=%d] [nthreads=%ld]\n", argv[0],n,nthreads);
		return -1;
	}
	const double a = 0.0;
	const double b = 1.0;
	...
	if (claimed < MPI_THREAD_FUNNELED) {
