 ...
 * jacobi_1d_blk.cpp
 *
 * 1D Jacobi iteration with blocking halo exchange
 *
 * Description:
 *   - The global 2D grid (rows * cols) is partitioned into contiguous row-blocks
 *     via MPI_Scatterv (each process gets myRows * cols elements)
 *   - At each iteration, each process exchanges its top and bottom "halo" rows
 *     with its immediate neighbors using two blocking MPI_Sendrecv calls.
 *   - Interior points (not depending on halos) are updated first; then boundary
 *     rows are updated once halos have been received.
 *   - The L2-error over each block is computed locally and reduced to all ranks
 *     with MPI_Allreduce.
 *   - Final blocks are gathered back to rank 0 via MPI_Gatherv, and the full grid
 *     is written to file.
 *
 * Usage:
 *   mpirun -n P ./jacobi_1d_blk inputFile rows cols outputFile errThreshold
 *
 * Notes:
 *   - Blocking Sendrecv ensures simple, deadlock-free halo exchanges.
 *   - Good for measuring baseline performance without overlap of communication
 *     and computation.
 */
#include <cstdlib>
#include <cstdio>
...
int main (int argc, char *argv[]){
	...
	if(argc < 6){
		...
		if(!myId){
			std::cout << "ERROR: The syntax of the program is ./jacobi_1d inputFile rows cols outputFile errThreshold"
					<< std::endl;
		}
		MPI_Abort(MPI_COMM_WORLD, -1);
	}
	...
	while(error > errThres){
		if(myId > 0){
			// Send/Receive the first row to the previous process
			MPI_Sendrecv(myData, cols, MPI_FLOAT, myId-1, 0,
						 prevRow, cols, MPI_FLOAT, myId-1, 1,
						 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}

		if(myId < numP-1){
			// Send/Receive the last row to the next process
			MPI_Sendrecv(&myData[(myRows-1)*cols], cols, MPI_FLOAT, myId+1, 1,
						 nextRow, cols, MPI_FLOAT, myId+1, 0,
						 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
