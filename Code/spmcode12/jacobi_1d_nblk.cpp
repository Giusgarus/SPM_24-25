 ...
 *   - At each iteration, each process posts non-blocking MPI_Irecv for top and bottom
 *     halo rows, then MPI_Isend of its boundary rows to neighbors.
 *   - While the halo communications are in flight, the process updates interior points
 ...
 *
 * Notes:
 *   - Non-blocking sends/receives allow overlap of communication and computation.
